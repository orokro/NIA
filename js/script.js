...
// Expose constants for debugger
window.EVENT_NEW_FRAME = EVENT_NEW_FRAME;
window.EVENT_AUDIO_BUFFER_FULL = EVENT_AUDIO_BUFFER_FULL;
window.EVENT_UNTIL_TICKS = EVENT_UNTIL_TICKS;
...
loader.load('3d/gb.glb', (gltf) => {
  gbModel = gltf.scene;
  scene.add(gbModel);
  
  // Center model
  const box = new THREE.Box3().setFromObject(gbModel);
  const center = box.getCenter(new THREE.Vector3());
  gbModel.position.sub(center);
...
class Emulator {
  ...
  constructor(module, romBuffer, extRamBuffer) {
    ...
    const hiddenCanvas = document.getElementById("mainCanvas");
    this.video = new Video(module, this.e, hiddenCanvas);
